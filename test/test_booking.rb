...
class BookingTest < Minitest::Test
  ...
  def test_booking_unavailable_resource_raises_error
    Booking.new(user: @user, resource: @resource)  # First booking
    assert_raises(BookingError) do
      Booking.new(user: @user, resource: @resource)  # Second booking should fail
    end
  end
  ...
  def test_cancelling_makes_resource_available_again
    booking = Booking.new(user: @user, resource: @resource)
    assert_equal false, @resource.available?
    booking.cancel
